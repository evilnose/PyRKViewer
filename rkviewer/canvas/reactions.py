...
from ..utils import Node, Vec2, pairwise
...
ToScrolledFn = Callable[[wx.Point], wx.Point]
...
class SpeciesBezier:
    ...
    def _recompute_curve(self):
        ...
        for i in range(MAXSEGS+1):
            tmp = Vec2()
            for j, point in enumerate((node_center, self.handle, self.centroid_handle,
                                       self.centroid)):
    ...
    def do_paint(self, gc: wx.GraphicsContext, to_scrolled_fn: ToScrolledFn):
        if self._dirty:
            self._recompute_curve()
            self._dirty = False
        # TODO convert to scrolled
        # Draw bezier curve
        gc.SetPen(wx.Pen(wx.BLACK, 2))
        path = gc.CreatePath()
        path.MoveToPoint(*to_scrolled_fn((self.bezier_points[0] * self.scale).to_wx_point()))
        for i in range(1, MAXSEGS+1):
            path.AddLineToPoint(*to_scrolled_fn((self.bezier_points[i] * self.scale).to_wx_point()))
        gc.StrokePath(path)

        # TODO clip source

    def do_paint_selected(self, gc: wx.GraphicsContext, to_scrolled_fn: ToScrolledFn):
        node_center = Vec2(to_scrolled_fn((self.node.center_point * self.scale).to_wx_point()))
        node_handle = Vec2(to_scrolled_fn((self.handle * self.scale).to_wx_point()))
        centroid = Vec2(to_scrolled_fn((self.centroid * self.scale).to_wx_point()))
        centroid_handle = Vec2(to_scrolled_fn((self.centroid_handle * self.scale).to_wx_point()))
        self.paint_handles(gc, node_center, node_handle, centroid, centroid_handle)
